...
class CommandExtractor(object):
    ...
    def replace_verb(self, sentence):
        ...
        for verb in	[' bring ', ' carry ', ' deliver ', ' take ']:
            if verb in sentence:
                ...
                # sentence = sentence.strip()
        for verb in	[' find ', ' look for ', ' identify ', 'detect']:
            if verb in sentence:
                sentence = sentence.replace(verb, ' find ')
                # return sentence.strip()
        for verb in	[' go to ', ' navigate to ', ' reach ', ' get into ', ' move to ', ' enter ']:
            if verb in sentence:
                sentence = sentence.replace(verb, ' go to ')
                # return sentence.strip()
        for verb in	[' take ', ' grasp ', ' get ',' pick ', ' grab ']:
            if verb in sentence:
                sentence = sentence.replace(verb, ' take ')
                # return sentence.strip()
        for verb in	[' tell ', ' say ', ' speak ', ' notify ', ' announce ']:
            # print sentence
            if verb in sentence:
                # print verb
    ...
    def __init__(self):
        ...
        self.objects = readFileToList(
            roslib.packages.get_pkg_dir('speech_processing') + '/command_config/GPSR/objects.txt')

        self.other_objects = readFileToList(
            roslib.packages.get_pkg_dir('speech_processing') + '/command_config/GPSR/other_objects.txt')
        ...
        self.rooms = readFileToList(
            roslib.packages.get_pkg_dir('speech_processing') + '/command_config/GPSR/rooms.txt')

        self.places = readFileToList(
            roslib.packages.get_pkg_dir('speech_processing') + '/command_config/GPSR/places.txt')
        ...
        self.names = readFileToList(
            roslib.packages.get_pkg_dir('speech_processing') + '/command_config/GPSR/name.txt')

        self.verbs = readFileToList(
            roslib.packages.get_pkg_dir('speech_processing') + '/command_config/GPSR/verbs.txt')
