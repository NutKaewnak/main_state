...
class GPSR(AbstractTask):
    ...
    def perform(self, perception_data):
        ...
        if self.state is 'init':
            ...
            if perception_data.device is 'VOICE' and self.command_extractor.isValidCommand(perception_data.input):
                self.state = 'confirm'
                self.command = data
            ...
            if device == Devices.voice:
                if 'yes' in data:
                    ...
                    for i in xrange(0, len(actions)):
                        ...
                        rospy.loginfo(' - %s,%s,%s' % (action.action, action.object if action.object else 'None',
            ...
            if device == Devices.voice and data in [obj for key in self.location_categories.keys() for obj in
                                                    self.location_categories[key]]:
            ...
            if device == Devices.voice:
                if 'yes' in data:
                    ...
                    self.speak('I will %s %s from %s' % (
                    self.current_action.action, self.current_action.object if self.current_action.object else '',
                    self.current_action.data if self.current_action.data else ''))
            ...
            if (device == Devices.recognition):
                ...
                for obj in objects:
                    print 'category : ' + obj.category + ' centroid : (' + str(obj.point.x) + "," + str(
                        obj.point.y) + "," + str(obj.point.z) + ")"
                for obj in objects:
